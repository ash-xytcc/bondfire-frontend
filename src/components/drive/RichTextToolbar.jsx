...
function Btn({ children, onClick, title }) {
  ...
  return <button className="btn bf-drive-toolbar-btn" type="button" onClick={onClick} title={title} style={{ padding: "3px 7px", fontSize: 11, lineHeight: 1 }}>{children}</button>;
}
...
export default function RichTextToolbar({ onBold, onItalic, onH1, onH2, onBullet, onQuote, onCode, onRule, onLink, onWikiLink, menuOpen = false, onToggleMenu, menuItems = [] }) {
  ...
  return (
    <div className="bf-drive-toolbar-wrap" style={{ position: "relative", marginBottom: 6 }}>
      <div className="card bf-drive-toolbar" style={{ padding: 5, display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center" }}>
        {hasFormatting ? <>
        ...
        <button className="btn bf-drive-toolbar-menu" type="button" onClick={onToggleMenu} title="View and document menu" style={{ marginLeft: "auto", padding: "3px 10px", fontSize: 14 }}>☰</button>
      </div>
      {menuOpen ? <div style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", minWidth: 220, background: "rgba(16,16,20,0.98)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: 6, boxShadow: "0 14px 36px rgba(0,0,0,0.45)", zIndex: 130, display: "grid", gap: 4 }}>
