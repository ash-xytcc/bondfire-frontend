...
function bodyForSave(post, status) {
  return {
    ...post,
    status,
  };
...
export default function OrgBulletinPage() {
  ...
  const saveWithStatus = React.useCallback(async (status) => {
    ...
    try {
      ...
      const data = await fetchJson(endpoint, {
        method,
        body: JSON.stringify(bodyForSave(current, status)),
      });
  ...
  if (loading) {
    return <div className="bulletin-admin">Loading bulletin editor…</div>;
  }

  if (error && !posts.length && !draft.title && !draft.body) {
    return <div className="bulletin-admin">Unable to load bulletin editor. {error}</div>;
  }
