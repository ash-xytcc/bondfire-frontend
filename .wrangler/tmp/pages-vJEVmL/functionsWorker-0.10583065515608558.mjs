...
__name(json, "json");
function bad(status, error3, extra) {
  return json({ ok: false, error: error3, ...extra || {} }, { status });
}
__name(bad, "bad");
function now() {
  return Date.now();
...
async function signJwt(secret, payload, ttlSeconds) {
  ...
  const enc = /* @__PURE__ */ __name((obj) => b64url(new TextEncoder().encode(JSON.stringify(obj))), "enc");
  const h = enc(header);
  const p = enc(body);
  const data = `${h}.${p}`;
...
// api/_lib/auth.js
function getDb(env2) {
  return env2?.BF_DB || env2?.DB || env2?.db || null;
}
__name(getDb, "getDb");
async function requireUser({ env: env2, request }) {
  ...
  const payload = await verifyJwt(env2.JWT_SECRET, token);
  if (!payload) return { ok: false, resp: bad(401, "UNAUTHORIZED") };
  return { ok: true, user: payload };
}
__name(requireUser, "requireUser");
async function requireOrgRole({ env: env2, request, orgId, minRole }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u;
...
async function getUserIdFromRequest(request, env2) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return null;
...
__name(getDb2, "getDb");
function getDriveBucket(env2) {
  return env2?.BF_DRIVE_BUCKET || env2?.DRIVE_BUCKET || env2?.BOND_FIRE_DRIVE_BUCKET || null;
}
__name(getDriveBucket, "getDriveBucket");
async function ensureDriveSchema(env2) {
  const db = getDb2(env2);
  if (!db) throw new Error("NO_DB_BINDING");
  if (env2.__bfDriveSchemaReady) return;
  const statements = [
  ...
  env2.__bfDriveSchemaReady = true;
}
...
async function listDriveTree(env2, orgId) {
  await ensureDriveSchema(env2);
  const db = getDb2(env2);
...
async function getFileRecord(env2, orgId, fileId, { includeData = false } = {}) {
  await ensureDriveSchema(env2);
  const db = getDb2(env2);
  const row = await db.prepare(
  ...
  const blob = await loadFileBlob(env2, orgId, row.id, row.storage_key, file.mime, file.name);
  return {
    ...file,
...
async function loadFileBlob(env2, orgId, fileId, storageKey, mime, name = "") {
  await ensureDriveSchema(env2);
  const bucket = getDriveBucket(env2);
...
async function saveFileBlob(env2, { orgId, fileId, storageKey, mime, dataUrl, textContent }) {
  await ensureDriveSchema(env2);
  const bucket = getDriveBucket(env2);
  const t = now();
  if (bucket && storageKey) {
...
async function deleteFileBlob(env2, { orgId, fileId, storageKey }) {
  await ensureDriveSchema(env2);
  const bucket = getDriveBucket(env2);
  if (bucket && storageKey) {
    try {
...
// api/orgs/[orgId]/drive/files/[id]/download.js
...
async function onRequestGet({ env: env2, request, params }) {
  const orgId = params.orgId;
  const fileId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  const file = await getFileRecord(env2, orgId, fileId, { includeData: false });
  ...
  const blob = await loadFileBlob(env2, orgId, fileId, file.storageKey, file.mime, file.name, file.encrypted);
  if (file.encrypted) {
...
// api/orgs/[orgId]/drive/files/[id].js
async function onRequestGet2({ env: env2, request, params }) {
  const orgId = params.orgId;
  const fileId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  const existingMeta = await getFileRecord(env2, orgId, fileId, { includeData: false });
  ...
  const file = existingMeta ? await getFileRecord(env2, orgId, fileId, { includeData: shouldIncludeData }) : null;
  if (!file) return bad(404, "NOT_FOUND");
  return json({ ok: true, file });
}
__name(onRequestGet2, "onRequestGet");
async function onRequestPatch({ env: env2, request, params }) {
  const orgId = params.orgId;
  const fileId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const db = getDb2(env2);
  ...
  if (body.dataUrl !== void 0 || body.textContent !== void 0 || body.mime !== void 0) {
    await saveFileBlob(env2, {
      orgId,
      fileId,
      storageKey: existing.storage_key,
      mime: nextMime,
      dataUrl: body.dataUrl === void 0 ? (await getFileRecord(env2, orgId, fileId, { includeData: true }))?.dataUrl || "" : String(body.dataUrl || ""),
      textContent: body.textContent === void 0 ? (await getFileRecord(env2, orgId, fileId, { includeData: true }))?.textContent || "" : String(body.textContent || "")
    });
  }
  ...
  const file = await getFileRecord(env2, orgId, fileId, { includeData: shouldIncludeData });
  return json({ ok: true, file });
}
__name(onRequestPatch, "onRequestPatch");
async function onRequestDelete({ env: env2, request, params }) {
  const orgId = params.orgId;
  const fileId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  ...
  if (!existing) return bad(404, "NOT_FOUND");
  await deleteFileBlob(env2, { orgId, fileId, storageKey: existing.storage_key || null });
...
// api/orgs/[orgId]/drive/folders/[id].js
async function onRequestPatch2({ env: env2, request, params }) {
  const orgId = params.orgId;
...
async function onRequestDelete2({ env: env2, request, params }) {
  const orgId = params.orgId;
  const folderId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  ...
  for (const row of fileRows.results || []) await deleteFileBlob(env2, { orgId, fileId: row.id, storageKey: row.storage_key || null });
...
// api/orgs/[orgId]/drive/notes/[id].js
async function onRequestGet3({ env: env2, request, params }) {
  const orgId = params.orgId;
  const noteId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const row = await getDb2(env2).prepare(`SELECT id, parent_id, title, content, tags, encrypted_blob, created_at, updated_at FROM drive_notes WHERE org_id = ? AND id = ?`).bind(orgId, noteId).first();
...
async function onRequestPatch3({ env: env2, request, params }) {
  const orgId = params.orgId;
  const noteId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
...
async function onRequestDelete3({ env: env2, request, params }) {
  const orgId = params.orgId;
  const noteId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  await getDb2(env2).prepare(`DELETE FROM drive_notes WHERE org_id = ? AND id = ?`).bind(orgId, noteId).run();
  return json({ ok: true, deleted: true, id: noteId });
}
__name(onRequestDelete3, "onRequestDelete");

// api/orgs/[orgId]/drive/templates/[id].js
async function onRequestPatch4({ env: env2, request, params }) {
  const orgId = params.orgId;
...
async function onRequestDelete4({ env: env2, request, params }) {
  const orgId = params.orgId;
  const templateId = params.id;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  await getDb2(env2).prepare(`DELETE FROM drive_templates WHERE org_id = ? AND id = ?`).bind(orgId, templateId).run();
  return json({ ok: true, deleted: true, id: templateId });
}
__name(onRequestDelete4, "onRequestDelete");

// api/_lib/publicPageStore.js
function slugify(input) {
  return String(input || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64);
}
__name(slugify, "slugify");
async function uniqueSlug(env2, base, orgId) {
  const cleanBase = slugify(base || "org") || "org";
  let trySlug = cleanBase;
  let n = 0;
  while (true) {
    const existingOrgId = await env2.BF_PUBLIC.get(`slug:${trySlug}`);
...
async function getPublicCfg(env2, orgId) {
  const raw = await env2.BF_PUBLIC.get(`org:${orgId}`);
  return raw ? JSON.parse(raw) : {};
}
__name(getPublicCfg, "getPublicCfg");
async function setPublicCfg(env2, orgId, cfg) {
  await env2.BF_PUBLIC.put(`org:${orgId}`, JSON.stringify(cfg));
}
__name(setPublicCfg, "setPublicCfg");
async function setSlugMapping(env2, slug, orgId) {
  const s = String(slug || "").trim().toLowerCase();
  await env2.BF_PUBLIC.put(`slug:${s}`, orgId);
}
__name(setSlugMapping, "setSlugMapping");
async function removeSlugMapping(env2, slug) {
  const s = String(slug || "").trim().toLowerCase();
  await env2.BF_PUBLIC.delete(`slug:${s}`);
}
__name(removeSlugMapping, "removeSlugMapping");
async function getOrgIdBySlug(env2, slug) {
  const s = String(slug || "").trim().toLowerCase();
  if (!s) return null;
  const orgId = await env2.BF_PUBLIC.get(`slug:${s}`);
  return orgId || null;
}
...
// api/_bf.js
function getDB(env2) {
  return env2.DB || env2.BF_DB || null;
}
__name(getDB, "getDB");
...
// api/p/[slug]/meetings/[meetingId]/rsvp.js
...
// api/orgs/[orgId]/meetings/[meetingId]/rsvp.js
...
async function getColumns(db) {
  const info3 = await db.prepare("PRAGMA table_info(meeting_rsvps)").all();
  const cols = /* @__PURE__ */ new Set();
  for (const r of info3?.results || []) cols.add(String(r?.name || ""));
  return cols;
}
...
async function onRequest(ctx) {
  const { request, env: env2, params } = ctx || {};
  ...
  const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
...
// api/orgs/[orgId]/drive/files.js
async function onRequestGet4({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const res = await getDb2(env2).prepare(`SELECT id, parent_id, name, mime, size, storage_key, encrypted, encrypted_blob, created_at, updated_at FROM drive_files WHERE org_id = ? ORDER BY LOWER(name) ASC`).bind(orgId).all();
...
async function onRequestPost2({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  ...
  const t = now();
  const bucket = getDriveBucket(env2);
  const persistBinaryUpload = /* @__PURE__ */ __name(async ({ name, mime, parentId, bytes, encrypted: encrypted2 = false, encryptedBlob = "" }) => {
    ...
    if (bucket && storageKey2) {
      try {
        ...
        return bad(500, "FILE_STORAGE_FAILED", { detail: String(error3?.message || error3 || "Bucket write failed") });
      }
    } else if (encrypted2) {
      await saveFileBlob(env2, { orgId, fileId: id, storageKey: storageKey2, mime: mime || "application/octet-stream", encryptedPayload: new TextDecoder().decode(bytes), encrypted: true });
    } else {
      ...
      await saveFileBlob(env2, { orgId, fileId: id, storageKey: storageKey2, mime, dataUrl, textContent, encrypted: false });
    }
    ...
    const createdFile2 = await getFileRecord(env2, orgId, id, { includeData: false });
  ...
  if (encrypted) {
    await saveFileBlob(env2, { orgId, fileId: id, storageKey, mime: "application/octet-stream", encryptedPayload: String(body.encryptedPayload || ""), encrypted: true });
  } else {
    await saveFileBlob(env2, { orgId, fileId: id, storageKey, mime: file.mime, dataUrl: String(body.dataUrl || ""), textContent: String(body.textContent || ""), encrypted: false });
  }
  const createdFile = await getFileRecord(env2, orgId, id, { includeData: false });
  return created("file", createdFile || file);
}
__name(onRequestPost2, "onRequestPost");

// api/orgs/[orgId]/drive/folders.js
async function onRequestGet5({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const res = await getDb2(env2).prepare(`SELECT id, parent_id, name, encrypted_blob, created_at, updated_at FROM drive_folders WHERE org_id = ? ORDER BY LOWER(name) ASC`).bind(orgId).all();
...
async function onRequestPost3({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  ...
  await getDb2(env2).prepare(`INSERT INTO drive_folders (id, org_id, parent_id, name, encrypted_blob, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(id, orgId, folder.parentId, folder.encryptedBlob ? "encrypted folder" : folder.name, folder.encryptedBlob || null, t, t).run();
  return created("folder", folder);
}
__name(onRequestPost3, "onRequestPost");

// api/orgs/[orgId]/drive/import.js
async function onRequestPost4({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
...
// api/orgs/[orgId]/drive/notes.js
async function onRequestGet6({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const res = await getDb2(env2).prepare(`SELECT id, parent_id, title, content, tags, encrypted_blob, created_at, updated_at FROM drive_notes WHERE org_id = ? ORDER BY updated_at DESC`).bind(orgId).all();
...
async function onRequestPost5({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  ...
  await getDb2(env2).prepare(`INSERT INTO drive_notes (id, org_id, parent_id, title, content, tags, encrypted_blob, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, orgId, note.parentId, note.encryptedBlob ? "encrypted note" : note.title, note.encryptedBlob ? "" : note.body, note.encryptedBlob ? "" : note.tags.join(","), note.encryptedBlob || null, t, t).run();
  return created("note", note);
}
__name(onRequestPost5, "onRequestPost");

// api/orgs/[orgId]/drive/templates.js
async function onRequestGet7({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
  const res = await getDb2(env2).prepare(`SELECT id, name, title, content, encrypted_blob, created_at, updated_at FROM drive_templates WHERE org_id = ? ORDER BY updated_at DESC`).bind(orgId).all();
...
async function onRequestPost6({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureDriveSchema(env2);
...
async function tableHasColumn(db, table3, col) {
  const info3 = await db.prepare(`PRAGMA table_info(${table3})`).all();
...
// api/orgs/[orgId]/newsletter/subscribers.js
...
async function onRequestGet8(ctx) {
  const { params, env: env2, request } = ctx;
  const orgId = String(params.orgId || "");
  ...
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!auth.ok) return auth.resp;
  const url = new URL(request.url);
...
// api/orgs/[orgId]/public/generate.js
function authOk(env2, request) {
  if (env2.BF_WRITE_LOCKED === "true") {
...
async function onRequestPost8({ env: env2, request, params }) {
  if (!authOk(env2, request)) {
    return Response.json({ ok: false, error: "UNAUTHORIZED" }, { status: 401 });
  }
  const orgId = params.orgId;
  const prev = await getPublicCfg(env2, orgId);
  const base = slugify(prev.title) || slugify(orgId) || "org";
  const final = await uniqueSlug(env2, base, orgId);
  if (prev.slug) {
    const mapped = await env2.BF_PUBLIC.get(`slug:${prev.slug}`);
    if (mapped === orgId) await removeSlugMapping(env2, prev.slug);
  }
  await setSlugMapping(env2, final, orgId);
  const saved = { ...prev, slug: final };
  await setPublicCfg(env2, orgId, saved);
  return Response.json({ ok: true, public: saved });
}
__name(onRequestPost8, "onRequestPost");

// api/orgs/[orgId]/public/get.js
function authOk2(env2, request) {
  if (env2.BF_WRITE_LOCKED === "true") {
    const auth = request.headers.get("authorization") || "";
    return auth.startsWith("Bearer ");
  }
...
__name(authOk2, "authOk");
async function onRequestGet9({ env: env2, request, params }) {
  if (!authOk2(env2, request)) {
    return Response.json({ ok: false, error: "UNAUTHORIZED" }, { status: 401 });
  }
  const orgId = params.orgId;
  const cfg = await getPublicCfg(env2, orgId);
  const cleaned = {
    enabled: !!cfg?.enabled,
...
// api/orgs/[orgId]/public/inbox.js
...
async function ensureTable2(db) {
  ...
  `).run();
  const info3 = await db.prepare(`PRAGMA table_info(public_inbox)`).all();
  const cols = new Set((info3?.results || []).map((r) => String(r.name || "").toLowerCase()));
...
function mapInboxItem(row) {
  ...
  let title2 = "Public intake";
  if (sourceKind === "get_help") title2 = "Get Help";
  else if (sourceKind === "volunteer") title2 = "Volunteer";
  else if (sourceKind === "offer_resources") title2 = "Offer Resources";
  else if (sourceKind === "inventory_request") title2 = "Inventory Request";
  else if (type === "rsvp") title2 = "Meeting RSVP";
  ...
  return { ...row, title: title2, details, extra };
}
...
async function onRequestPut(context2) {
  const { env: env2, params, request } = context2;
  try {
    const orgId = String(params?.orgId || "").trim();
    if (!orgId) return json3({ ok: false, error: "BAD_ORG" }, 400);
    const body = await request.json().catch(() => ({}));
...
// api/orgs/[orgId]/public/save.js
function authOk3(env2, request) {
  if (env2.BF_WRITE_LOCKED === "true") {
    const auth = request.headers.get("authorization") || "";
    return auth.startsWith("Bearer ");
  }
...
async function onRequestPost9({ env: env2, request, params }) {
  if (!authOk3(env2, request)) {
    return Response.json({ ok: false, error: "UNAUTHORIZED" }, { status: 401 });
  }
  const orgId = params.orgId;
  const body = await request.json().catch(() => ({}));
  ...
  if (typeof slug === "string" && slug.trim()) {
    ...
    const base = slugify(title2) || slugify(orgId) || "org";
    const final = await uniqueSlug(env2, base, orgId);
    await setSlugMapping(env2, final, orgId);
    newSlug = final;
  }
  ...
  await setPublicCfg(env2, orgId, cleaned);
  return Response.json({ ok: true, public: cleaned });
}
__name(onRequestPost9, "onRequestPost");

// api/orgs/[orgId]/studio/state.js
...
async function onRequestGet11({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  await ensureStudioTables(env2.BF_DB);
  const docsRes = await env2.BF_DB.prepare(
    `SELECT id, name, encrypted_blob, key_version, created_at, updated_at
     FROM studio_docs
     WHERE org_id = ?
     ORDER BY updated_at DESC`
  ).bind(orgId).all();
  const blocksRes = await env2.BF_DB.prepare(
    `SELECT id, name, encrypted_blob, key_version, created_at, updated_at
     FROM studio_blocks
     WHERE org_id = ?
     ORDER BY updated_at DESC`
...
async function onRequestPost10({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!auth.ok) return auth.resp;
  await ensureStudioTables(env2.BF_DB);
  ...
  const t = now();
  const keyVersion = await getOrgCryptoKeyVersion(env2.BF_DB, orgId);
  ...
  for (const item of docs) {
    ...
    if (!id) continue;
    const exists = await env2.BF_DB.prepare(
      "SELECT id FROM studio_docs WHERE id = ? AND org_id = ?"
    ).bind(id, orgId).first();
    if (exists?.id) {
      await env2.BF_DB.prepare(
        `UPDATE studio_docs
  ...
  if (keepDocIds.length) {
    ...
    await env2.BF_DB.prepare("DELETE FROM studio_docs WHERE org_id = ?").bind(orgId).run();
  }
  if (keepBlockIds.length) {
    ...
    await env2.BF_DB.prepare("DELETE FROM studio_blocks WHERE org_id = ?").bind(orgId).run();
  }
  return json({ ok: true, docs_saved: keepDocIds.length, blocks_saved: keepBlockIds.length });
}
__name(onRequestPost10, "onRequestPost");

// api/orgs/[orgId]/studio/updates.js
...
__name(sse, "sse");
async function onRequestGet12({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      let lastSig = "";
      try {
        lastSig = await latestStudioSig(env2.BF_DB, orgId);
      ...
      const heartbeat = setInterval(async () => {
        if (closed) return;
        try {
          const sig = await latestStudioSig(env2.BF_DB, orgId);
          if (sig !== lastSig) {
            lastSig = sig;
      ...
      request.signal?.addEventListener("abort", abort2, { once: true });
    },
    cancel() {
    }
  });
...
// api/_lib/zkSchema.js
async function tableInfo(db, table3) {
  const rows = await db.prepare(`PRAGMA table_info(${table3})`).all();
  return rows?.results || [];
}
__name(tableInfo, "tableInfo");
async function ensureZkSchema2(env2) {
  const db = getDb(env2);
  if (!db) throw new Error("NO_DB_BINDING");
  await db.prepare(
    `CREATE TABLE IF NOT EXISTS org_crypto (
...
__name(ensureZkSchema2, "ensureZkSchema");
...
__name(getOrgKeyVersion2, "getOrgKeyVersion");
...
// api/orgs/[orgId]/zk/recovery.js
async function getRecoveryTableCaps(db) {
  const info3 = await db.prepare("PRAGMA table_info(org_key_recovery)").all();
  const names = new Set((info3?.results || []).map((c) => c.name));
  return {
...
async function onRequestGet13(ctx) {
  try {
    const { env: env2, request, params } = ctx;
    const orgId = String(params.orgId);
    const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
    if (!gate.ok) return gate.resp;
    const userId = String(gate.user.sub);
    const { db } = await ensureZkSchema2(env2);
...
// api/orgs/[orgId]/zk/rotate.js
async function onRequestPost12({ env: env2, request, params }) {
  try {
    const orgId = String(params.orgId);
    const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "owner" });
    if (!gate.ok) return gate.resp;
    const { db } = await ensureZkSchema2(env2);
...
// api/orgs/[orgId]/zk/status.js
async function onRequestGet14({ env: env2, request, params }) {
  try {
    const orgId = String(params.orgId);
    const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
    if (!gate.ok) return gate.resp;
    const { db } = await ensureZkSchema2(env2);
    const version2 = await ensureOrgCryptoRow(db, orgId);
    const caps = await orgKeyWrappedCapabilities(db);
...
async function onRequestPost13({ env: env2, request, params }) {
  try {
    const orgId = String(params.orgId);
    const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
    if (!gate.ok) return gate.resp;
    const body = await readJSON(request);
    const { db } = await ensureZkSchema2(env2);
    const caps = await orgKeyWrappedCapabilities(db);
    const version2 = await ensureOrgCryptoRow(db, orgId);
    const keyVersion = Number(body?.key_version || version2 || 1);
...
// api/orgs/[orgId]/zk/wrapped.js
async function onRequestGet15({ env: env2, request, params }) {
  try {
    const orgId = String(params.orgId);
    const gate = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
    if (!gate.ok) return gate.resp;
    ...
    return ok({
      org_id: orgId,
      key_version: row?.key_version ? Number(row.key_version) : version2,
...
// api/orgs/[orgId]/newsletter/subscribe.js
...
// api/p/[slug]/newsletter/subscribe.js
async function onRequestPost14(context2) {
  const slug = context2?.params?.slug ? String(context2.params.slug) : "";
  if (!slug) return bad(400, "Missing slug");
  const orgId = await getOrgIdBySlug(context2.env, slug);
  if (!orgId) return bad(404, "Not found");
  const ctx2 = { ...context2, params: { ...context2.params || {}, orgId } };
  return onRequest2(ctx2);
}
__name(onRequestPost14, "onRequestPost");

// api/orgs/[orgId]/meetings/[meetingId].js
...
async function getRsvpCounts(db, orgId, meetingId) {
  ...
  for (const row of memberRows?.results || []) {
    ...
    const count3 = Number(row?.c || 0);
    if (status === "yes" || status === "maybe" || status === "no") member[status] += count3;
  }
  for (const row of publicRows?.results || []) {
    const status = String(row?.status || "").toLowerCase();
    const count3 = Number(row?.c || 0);
    if (status === "yes" || status === "maybe" || status === "no") pub[status] += count3;
  }
...
async function onRequestGet16({ env: env2, request, params }) {
  const orgId = params.orgId;
  const meetingId = params.meetingId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  await ensureMeetingsPublicColumn(env2.BF_DB);
  await ensurePublicMeetingRsvpsTable(env2.BF_DB);
  const row = await env2.BF_DB.prepare(
    `SELECT id, title, starts_at, ends_at, location, agenda, notes, is_public, created_at, updated_at
     FROM meetings
     WHERE id = ? AND org_id = ?`
  ).bind(meetingId, orgId).first();
  if (!row) return bad(404, "NOT_FOUND");
  const rsvp_counts = await getRsvpCounts(env2.BF_DB, orgId, meetingId);
  return json({ ok: true, meeting: { ...row, rsvp_counts } });
}
__name(onRequestGet16, "onRequestGet");
async function onRequestPut2({ env: env2, request, params }) {
  const orgId = params.orgId;
  const meetingId = params.meetingId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  await ensureMeetingsPublicColumn(env2.BF_DB);
  const body = await request.json().catch(() => ({}));
  await env2.BF_DB.prepare(
...
async function onRequestDelete7({ env: env2, request, params }) {
  const orgId = params.orgId;
  const meetingId = params.meetingId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!a.ok) return a.resp;
  await env2.BF_DB.prepare("DELETE FROM meetings WHERE id = ? AND org_id = ?").bind(meetingId, orgId).run();
  return json({ ok: true });
}
...
async function aesGcmDecrypt(encObj, secretString) {
  const key = await importAesKeyFromString(secretString);
  const fromB642 = /* @__PURE__ */ __name((s) => new Uint8Array(atob(String(s)).split("").map((c) => c.charCodeAt(0))), "fromB64");
...
async function rateLimit({ env: env2, key, limit = 10, windowSec = 600 } = {}) {
  const now4 = Date.now();
...
async function issueAccessToken(env2, user, ttlSec = 60 * 15) {
  return signJwt(env2.JWT_SECRET, { sub: user.id, email: user.email, name: user.name }, ttlSec);
}
__name(issueAccessToken, "issueAccessToken");

// api/auth/login/mfa.js
...
async function onRequestPost15({ env: env2, request }) {
  ...
  const rl = await rateLimit({ env: env2, key: `mfa:${ip}:${challengeId}`, limit: 12, windowSec: 600 });
  if (!rl.ok) return bad(429, "RATE_LIMIT", { retry_after: rl.retry_after });
  const ch = await env2.BF_DB.prepare("SELECT id, user_id, expires_at, verified FROM login_mfa_challenges WHERE id = ?").bind(challengeId).first();
  ...
  const mfa = await env2.BF_DB.prepare("SELECT totp_secret_encrypted, mfa_enabled FROM user_mfa WHERE user_id = ?").bind(ch.user_id).first();
  if (!mfa || Number(mfa.mfa_enabled) !== 1) return bad(401, "MFA_NOT_ENABLED");
  let ok2 = false;
  if (recoveryCode) {
    const pepper = env2.RECOVERY_PEPPER || env2.JWT_SECRET || "pepper";
    const h = await hashRecovery(recoveryCode, pepper);
    const row = await env2.BF_DB.prepare("SELECT id, used FROM user_mfa_recovery_codes WHERE user_id = ? AND code_hash = ?").bind(ch.user_id, h).first();
    if (row && Number(row.used) === 0) {
      await env2.BF_DB.prepare("UPDATE user_mfa_recovery_codes SET used = 1 WHERE id = ?").bind(row.id).run();
      ok2 = true;
    }
  }
  if (!ok2) {
    if (!code) return bad(400, "MISSING_CODE");
    const encKey = env2.MFA_ENC_KEY || env2.JWT_SECRET;
  ...
  await env2.BF_DB.prepare("UPDATE login_mfa_challenges SET verified = 1 WHERE id = ?").bind(challengeId).run();
  const user = await env2.BF_DB.prepare("SELECT id, email, name FROM users WHERE id = ?").bind(ch.user_id).first();
  if (!user) return bad(401, "INVALID_LOGIN");
  const accessToken = await issueAccessToken(env2, user, 60 * 15);
  ...
  const isProd = (env2?.ENV || env2?.NODE_ENV || "").toLowerCase() === "production";
...
async function onRequestPost16({ request, env: env2 }) {
  try {
    const u = await requireUser({ env: env2, request });
...
// api/matrix/e2ee/get.js
async function onRequestGet17({ env: env2, request }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const url = new URL(request.url);
  const key = url.searchParams.get("key");
  if (!key) return bad(400, "MISSING_KEY");
  const v = await env2.BF_E2EE.get(`u:${u.user.sub}:${key}`);
  return json({ ok: true, value: v ? JSON.parse(v) : null });
}
__name(onRequestGet17, "onRequestGet");

// api/matrix/e2ee/put.js
async function onRequestPost17({ env: env2, request }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const body = await request.json().catch(() => ({}));
  const key = String(body.key || "");
  if (!key) return bad(400, "MISSING_KEY");
  await env2.BF_E2EE.put(`u:${u.user.sub}:${key}`, JSON.stringify(body.value ?? null));
  return json({ ok: true });
}
__name(onRequestPost17, "onRequestPost");

// api/auth/mfa/confirm.js
...
__name(hashRecovery2, "hashRecovery");
...
async function onRequest4(context2) {
  ...
  try {
    ...
    const issuer = env2.TOTP_ISSUER || "Bondfire";
...
async function getPublicForm(env2, fileId) {
  await ensureDriveSchema(env2);
  const db = getDb2(env2);
  const row = await db.prepare(`SELECT id, org_id, name, mime, storage_key FROM drive_files WHERE id = ?`).bind(fileId).first();
  if (!row) return null;
  const blob = await loadFileBlob(env2, row.org_id, row.id, row.storage_key || null, row.mime || "application/octet-stream", row.name || "");
...
async function onRequestGet18({ env: env2, request, params }) {
  const fileId = params.id;
  const token = new URL(request.url).searchParams.get("token") || "";
  const record = await getPublicForm(env2, fileId);
...
async function onRequestPost18({ env: env2, request, params }) {
  const fileId = params.id;
  const record = await getPublicForm(env2, fileId);
  if (!record) return bad(404, "NOT_FOUND");
  const body = await request.json().catch(() => ({}));
  ...
  await getDb2(env2).prepare(`UPDATE drive_files SET updated_at = ? WHERE id = ?`).bind(now(), fileId).run();
  return json({ ok: true, submitted: true, responseId: response.id });
}
__name(onRequestPost18, "onRequestPost");

// api/orgs/[orgId]/activity.js
async function onRequestGet19({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  const url = new URL(request.url);
  const limit = Math.min(100, Math.max(1, Number(url.searchParams.get("limit") || 20)));
  const res = await env2.BF_DB.prepare(
...
async function onRequestPost19({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
...
async function onRequestGet20({ env: env2, request, params }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const orgId = String(params?.orgId || "");
  ...
  const role = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
...
async function onRequestPost20({ env: env2, request, params }) {
  const m = requireMethod(request, "POST");
  if (m) return m;
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  ...
  if (!db) return bad(500, "NO_DB_BINDING");
  await ensureZkColumns(db);
  const role = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!role.ok) return role.resp;
  const body = await readJSON(request);
  ...
  if (keyVersion != null) {
    const info3 = await db.prepare("PRAGMA table_info(org_crypto)").all();
    const cols = new Set((info3?.results || []).map((r) => r.name));
...
// api/orgs/[orgId]/dashboard.js
async function safeFirst(env2, sql, binds = [], fallback = null) {
  try {
    const stmt = env2.BF_DB.prepare(sql);
...
async function safeAll(env2, sql, binds = [], fallback = []) {
  try {
    const stmt = env2.BF_DB.prepare(sql);
...
async function onRequestGet21({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
...
// api/orgs/[orgId]/drive/index.js
async function onRequestGet22({ env: env2, request, params }) {
  const orgId = params.orgId;
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!auth.ok) return auth.resp;
  const tree = await listDriveTree(env2, orgId);
  return json({ ok: true, ...tree });
}
__name(onRequestGet22, "onRequestGet");

// api/_lib/activity.js
async function logActivity(env2, { orgId, kind, message, actorUserId = null }) {
  try {
    if (!env2?.BF_DB) return;
...
__name(getOrgCryptoKeyVersion2, "getOrgCryptoKeyVersion");
...
async function onRequestGet23({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  await ensureInventoryParsTable(env2.BF_DB);
  const res = await env2.BF_DB.prepare(
...
async function onRequestPost21({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
  ...
  const keyVersion = body.encrypted_blob ? await getOrgCryptoKeyVersion2(env2.BF_DB, orgId) : null;
  await env2.BF_DB.prepare(
  ...
  const created2 = await env2.BF_DB.prepare(
...
async function onRequestPut3({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
  ...
  if (Object.prototype.hasOwnProperty.call(body, "par")) {
    ...
    if (Number.isFinite(par) && par > 0) {
      ...
      await env2.BF_DB.prepare(`DELETE FROM inventory_pars WHERE org_id = ? AND inventory_id = ?`).bind(orgId, id).run();
    }
  }
  try {
    await logActivity(env2, {
  ...
  const item = await env2.BF_DB.prepare(
    `SELECT i.id, i.org_id, i.name, i.qty, i.unit, i.category, i.location, i.notes,
            i.encrypted_notes, i.encrypted_blob, i.key_version,
            i.is_public, i.created_at, i.updated_at,
            ip.par
...
async function onRequestDelete8({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!a.ok) return a.resp;
  const url = new URL(request.url);
  ...
  if (!id) return bad(400, "MISSING_ID");
  const prev = await env2.BF_DB.prepare(
    "SELECT name FROM inventory WHERE id = ? AND org_id = ?"
  ).bind(id, orgId).first();
  const shortId = /* @__PURE__ */ __name((x) => typeof x === "string" && x.length > 12 ? `${x.slice(0, 8)}\u2026${x.slice(-4)}` : x || "", "shortId");
  const name = String(prev?.name || "").trim();
  const label = name || shortId(id);
  await ensureInventoryParsTable(env2.BF_DB);
  await env2.BF_DB.prepare("DELETE FROM inventory_pars WHERE org_id = ? AND inventory_id = ?").bind(orgId, id).run();
  await env2.BF_DB.prepare("DELETE FROM inventory WHERE id = ? AND org_id = ?").bind(id, orgId).run();
  logActivity(env2, {
    orgId,
...
async function onRequest5(ctx) {
  ...
  if (!db) return bad(500, "NO_DB_BINDING");
  const orgId = params.orgId;
  const roleCheck = await requireOrgRole({ env: env2, request, orgId, minRole: "owner" });
...
__name(getOrgCryptoKeyVersion3, "getOrgCryptoKeyVersion");
...
__name(ensureMeetingsPublicColumn2, "ensureMeetingsPublicColumn");
async function onRequestGet25({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  await ensureMeetingsPublicColumn2(env2.BF_DB);
  await ensureMeetingsZkColumns(env2.BF_DB);
  const res = await env2.BF_DB.prepare(
...
async function onRequestPost23({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
  const title2 = String(body.title || "").trim();
  if (!title2) return bad(400, "MISSING_TITLE");
  const id = uuid();
  const t = now();
  ...
  if (body.encrypted_blob) {
    keyVersion = await getOrgCryptoKeyVersion3(env2.BF_DB, orgId);
  }
  await env2.BF_DB.prepare(
  ...
  try {
    await logActivity(env2, {
      ...
      message: `meeting created: ${title2}`,
      actorUserId: a?.user?.sub || null
    });
  } catch (e) {
    console.error("ACTIVITY_FAIL", e);
...
async function onRequestPut4({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
...
async function onRequestDelete9({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!a.ok) return a.resp;
  const url = new URL(request.url);
  const id = url.searchParams.get("id");
  if (!id) return bad(400, "MISSING_ID");
  await env2.BF_DB.prepare("DELETE FROM meetings WHERE id = ? AND org_id = ?").bind(id, orgId).run();
  const prev = await env2.BF_DB.prepare(
    "SELECT title FROM meetings WHERE id = ? AND org_id = ?"
  ).bind(id, orgId).first();
  const shortId = /* @__PURE__ */ __name((x) => typeof x === "string" && x.length > 12 ? `${x.slice(0, 8)}\u2026${x.slice(-4)}` : x || "", "shortId");
  const title2 = String(prev?.title || "").trim();
  const label = title2 || shortId(id);
  logActivity(env2, {
    orgId,
...
__name(getOrgCryptoKeyVersion4, "getOrgCryptoKeyVersion");
function asString(v) {
  if (v == null) return "";
...
async function safeLog(env2, payload) {
  try {
    await logActivity(env2, payload);
  } catch (e) {
...
async function onRequestGet26({ env: env2, request, params }) {
  await ensureNeedsZkColumns(env2.BF_DB);
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  const r = await env2.BF_DB.prepare(
    `SELECT
       id,
       title,
       description,
...
async function onRequestPost24({ env: env2, request, params }) {
  await ensureNeedsZkColumns(env2.BF_DB);
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  const body = await request.json().catch(() => ({}));
  const title2 = asString(body.title).trim();
  if (!title2) return bad(400, "Title is required");
  ...
  let keyVersion = null;
  if (body.encrypted_blob) {
    keyVersion = await getOrgCryptoKeyVersion4(env2.BF_DB, orgId);
  }
  await env2.BF_DB.prepare(
...
async function onRequestPut5({ env: env2, request, params }) {
  await ensureNeedsZkColumns(env2.BF_DB);
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  ...
  const existing = await env2.BF_DB.prepare(
...
async function onRequestDelete10({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!a.ok) return a.resp;
  const url = new URL(request.url);
  ...
  id = asString(id).trim();
  if (!id) return bad(400, "id is required");
  const before = await env2.BF_DB.prepare(
    `SELECT title FROM needs WHERE org_id = ? AND id = ?`
  ).bind(orgId, id).first();
  await env2.BF_DB.prepare(`DELETE FROM needs WHERE org_id = ? AND id = ?`).bind(orgId, id).run();
  await safeLog(env2, {
    orgId,
    kind: "need.deleted",
...
__name(getOrgCryptoKeyVersion5, "getOrgCryptoKeyVersion");
...
async function onRequestGet27({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "viewer" });
  if (!a.ok) return a.resp;
  await ensurePeopleZkColumns(env2.BF_DB);
  const res = await env2.BF_DB.prepare(
...
async function onRequestPost25({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  await ensurePeopleZkColumns(env2.BF_DB);
  ...
  let keyVersion = null;
  if (body.encrypted_blob) {
    keyVersion = await getOrgCryptoKeyVersion5(env2.BF_DB, orgId);
  }
  await env2.BF_DB.prepare(
...
async function onRequestPut6({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "member" });
  if (!a.ok) return a.resp;
  await ensurePeopleZkColumns(env2.BF_DB);
...
async function onRequestDelete11({ env: env2, request, params }) {
  const orgId = params.orgId;
  const a = await requireOrgRole({ env: env2, request, orgId, minRole: "admin" });
  if (!a.ok) return a.resp;
  const url = new URL(request.url);
  ...
  const name = String(prev?.name || "").trim();
  const label = name || shortId(id);
  await env2.BF_DB.prepare("DELETE FROM people WHERE id = ? AND org_id = ?").bind(id, orgId).run();
  logActivity(env2, {
    orgId,
    kind: "person.deleted",
...
async function onRequestGet28({ env: env2, request, params }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const orgId = String(params?.orgId || "");
  if (!orgId) return bad(400, "MISSING_ORG_ID");
...
async function onRequestPost26({ env: env2, request, params }) {
  const m = requireMethod(request, "POST");
  if (m) return m;
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
...
async function onRequestDelete12({ env: env2, request, params }) {
...
// api/p/[slug]/intake.js
async function getOrgIdBySlug2(env2, slug) {
  const s = String(slug || "").trim();
  if (!s) return null;
  const orgId = await env2.BF_PUBLIC.get(`slug:${s}`);
  return orgId || null;
}
__name(getOrgIdBySlug2, "getOrgIdBySlug");
...
async function onRequestPost27({ env: env2, params, request }) {
  const slug = String(params?.slug || "").trim();
  if (!slug) return bad(400, "MISSING_SLUG");
  const orgId = await getOrgIdBySlug2(env2, slug);
  if (!orgId) return bad(404, "NOT_FOUND");
  const db = getDB(env2);
  if (!db) return bad(500, "DB_NOT_CONFIGURED");
...
// api/public/[slug]/inventory.js
async function getOrgIdBySlug3(env2, slug) {
  const s = String(slug || "").trim();
  if (!s) return null;
  const orgId = await env2.BF_PUBLIC.get(`slug:${s}`);
  return orgId || null;
}
__name(getOrgIdBySlug3, "getOrgIdBySlug");
...
async function onRequestGet29({ env: env2, params }) {
  try {
    const slug = String(params?.slug || "").trim();
    ...
    const orgId = await getOrgIdBySlug3(env2, slug);
...
// api/public/[slug]/meetings.js
async function onRequestGet30({ env: env2, params }) {
  ...
  const pub = await getPublicCfg(env2, orgId);
...
// api/public/[slug]/needs.js
async function onRequestGet31({ env: env2, params }) {
  const slug = params.slug;
  const orgId = await env2.BF_PUBLIC.get(`slug:${slug}`);
  if (!orgId) return json({ ok: false, needs: [] }, { status: 404 });
  const cfgRaw = await env2.BF_PUBLIC.get(`org:${orgId}`);
...
async function onRequest8(ctx) {
  ...
  try {
    ...
    if (request.method === "PUT") {
      ...
      const title2 = type !== void 0 ? type : void 0;
      ...
      const sets = [];
      const vals = [];
      const add = /* @__PURE__ */ __name((col, val) => {
        sets.push(`${col}=?`);
        vals.push(val);
      }, "add");
      ...
      if (title2 !== void 0) add("title", title2 || "");
...
// api/p/[slug]/meetings.js
async function getOrgIdBySlug4(env2, slug) {
  const s = String(slug || "").trim();
  if (!s) return null;
  const orgId = await env2.BF_PUBLIC.get(`slug:${s}`);
  return orgId || null;
}
__name(getOrgIdBySlug4, "getOrgIdBySlug");
...
async function onRequest9(ctx) {
  ...
  const orgId = await getOrgIdBySlug4(env2, slug);
...
__name(ensurePledgesTable2, "ensurePledgesTable");
function now3() {
  return Date.now();
}
__name(now3, "now");
...
__name(toNumOrNull2, "toNumOrNull");
...
__name(normalizeLegacyContact2, "normalizeLegacyContact");
async function getOrgIdBySlug5(env2, slug) {
  const s = String(slug || "").trim();
  if (!s) return null;
  const orgId = await env2.BF_PUBLIC.get(`slug:${s}`);
  return orgId || null;
}
__name(getOrgIdBySlug5, "getOrgIdBySlug");
async function onRequest10(ctx) {
  ...
  const orgId = await getOrgIdBySlug5(env2, slug);
  if (!orgId) return err(404, "NOT_FOUND");
  try {
...
// api/auth/keys.js
async function onRequestGet32({ env: env2, request }) {
  const auth = await requireUser({ env: env2, request });
  if (!auth.ok) return auth.resp;
...
async function onRequestPost28({ env: env2, request }) {
  const auth = await requireUser({ env: env2, request });
  if (!auth.ok) return auth.resp;
  const userId = auth.user?.sub;
  if (!userId) return bad(401, "UNAUTHORIZED");
...
async function onRequestPost29({ env: env2, request }) {
  const ip = request.headers.get("cf-connecting-ip") || request.headers.get("x-forwarded-for") || "";
  const body = await request.json().catch(() => ({}));
  const email = String(body.email || "").trim().toLowerCase();
  const password = String(body.password || "");
  const rl = await rateLimit({ env: env2, key: `login:${ip}:${email}`, limit: 12, windowSec: 600 });
  if (!rl.ok) return bad(429, "RATE_LIMIT", { retry_after: rl.retry_after });
  const user = await env2.BF_DB.prepare(
    "SELECT id, email, name, password_hash FROM users WHERE email = ?"
  ).bind(email).first();
  if (!user) return bad(401, "INVALID_LOGIN");
  const ok2 = await verifyPass(password, user.password_hash);
  if (!ok2) return bad(401, "INVALID_LOGIN");
  const mfa = await env2.BF_DB.prepare(
...
// api/auth/logout.js
async function onRequestPost30({ env: env2, request }) {
  const rt = getCookie(request, "bf_rt");
  if (rt && env2?.BF_DB) {
    const h = await sha256Hex2(rt);
    await env2.BF_DB.prepare("DELETE FROM refresh_tokens WHERE token_hash = ?").bind(h).run();
  }
  const isProd = (env2?.ENV || env2?.NODE_ENV || "").toLowerCase() === "production";
  const resp = ok({});
...
// api/auth/logout_all.js
async function onRequestPost31({ env: env2, request }) {
  const auth = await requireUser({ env: env2, request });
  if (!auth.ok) return auth.resp;
  const userId = auth.user?.sub;
...
// api/auth/me.js
async function onRequestGet33({ env: env2, request }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const row = await env2.BF_DB.prepare(
...
// api/auth/public-key.js
async function onRequestPost32({ env: env2, request }) {
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
  const body = await request.json().catch(() => ({}));
...
// api/auth/refresh.js
async function onRequestPost33({ env: env2, request }) {
  if (!env2?.BF_DB) return bad(500, "BF_DB_MISSING");
  const rt = getCookie(request, "bf_rt");
  ...
  if (!row) {
    ...
    const isProd2 = (env2?.ENV || env2?.NODE_ENV || "").toLowerCase() === "production";
    for (const c of clearAuthCookieHeaders({ isProd: isProd2 })) resp2.headers.append("set-cookie", c);
    return resp2;
  }
  if (Number(row.expires_at) < Date.now()) {
    await env2.BF_DB.prepare("DELETE FROM refresh_tokens WHERE id = ?").bind(row.id).run();
    const resp2 = bad(401, "REFRESH_EXPIRED");
    const isProd2 = (env2?.ENV || env2?.NODE_ENV || "").toLowerCase() === "production";
    for (const c of clearAuthCookieHeaders({ isProd: isProd2 })) resp2.headers.append("set-cookie", c);
    return resp2;
  }
  const user = await env2.BF_DB.prepare("SELECT id, email, name FROM users WHERE id = ?").bind(row.user_id).first();
...
async function onRequestPost34({ env: env2, request }) {
  try {
    const body = await request.json().catch(() => ({}));
    const email = String(body.email || "").trim().toLowerCase();
    const name = String(body.name || "").trim();
    ...
    if (!env2.BF_DB) return bad(500, "BF_DB_MISSING");
    if (!env2.JWT_SECRET) return bad(500, "JWT_SECRET_MISSING");
    const exists = await env2.BF_DB.prepare("SELECT id FROM users WHERE email = ?").bind(email).first();
...
async function onRequestGet34({ env: env2, request }) {
  const auth = await requireUser({ env: env2, request });
  if (!auth.ok) return auth.resp;
  const userId = auth.user?.sub;
...
async function onRequestPost35({ request, env: env2 }) {
  if (!env2.JWT_SECRET) return bad(500, "JWT_SECRET_MISSING");
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
...
// api/orgs/create.js
async function onRequestPost36({ request, env: env2 }) {
  if (!env2.JWT_SECRET) return bad(500, "JWT_SECRET_MISSING");
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
...
// api/pixabay/search.js
async function onRequestGet35(context2) {
  const { request, env: env2 } = context2;
  const apiKey = env2.PIXABAY_API_KEY || env2.VITE_PIXABAY_API_KEY;
  if (!apiKey) {
...
// api/orgs/[orgId].js
async function onRequestDelete13({ env: env2, request, params }) {
  const orgId = params?.orgId;
  if (!orgId) return bad(400, "MISSING_ORG_ID");
  const auth = await requireOrgRole({ env: env2, request, orgId, minRole: "owner" });
  if (!auth.ok) return auth.resp;
  const db = getDb(env2);
  if (!db) return bad(500, "NO_DB_BINDING");
...
// api/public/[slug].js
async function onRequestGet36({ env: env2, params }) {
  const slug = params.slug;
  const orgId = await env2.BF_PUBLIC.get(`slug:${slug}`);
  if (!orgId) {
...
// api/orgs/index.js
async function onRequestGet38({ env: env2, request }) {
  if (!env2.JWT_SECRET) return bad(500, "JWT_SECRET_MISSING");
  const u = await requireUser({ env: env2, request });
  if (!u.ok) return u.resp;
...
async function onRequest11(context2) {
  ...
  if (segments.length === 2 && segments[0] === "invites" && segments[1] === "redeem") {
    if (request.method === "POST") return withCors(await onRequestPost35(context2));
    return json6({ ok: false, error: "Method not allowed" }, 405);
  }
  return json6(
...
// ../.wrangler/tmp/pages-vJEVmL/functionsRoutes-0.13167943704959018.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-EKtvCO/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-EKtvCO/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.10583065515608558.mjs.map
