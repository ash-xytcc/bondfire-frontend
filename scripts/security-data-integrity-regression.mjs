...
async function privateStoreAndAuthorizationChecks() {
  ...
  res=await privateRecords({env,request:request(`https://test.local/api/orgs/org-sec/needs/${cookieNeed}`,{method:'POST',cookie,body:{id:cookieNeed,ciphertext:cookieCipher,revision:0}}),orgId:'org-sec',kind:'needs',id:cookieNeed});
  assert.equal(res.status,403);
  assert.equal((await json(res)).error,'CSRF_REQUIRED');
  res=await privateRecords({env,request:request(`https://test.local/api/orgs/org-sec/needs/${cookieNeed}`,{method:'POST',cookie,csrf:'csrf-regression',body:{id:cookieNeed,ciphertext:cookieCipher,revision:0}}),orgId:'org-sec',kind:'needs',id:cookieNeed});
  assert.equal(res.status,200);
