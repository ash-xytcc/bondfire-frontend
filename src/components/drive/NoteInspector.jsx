  ...
  return (
    <div className="card" style={{ padding: 8, background: "rgba(11,11,11,0.96)", backdropFilter: "blur(8px)" }}>
        ...
        {postMeta ? (
          <div style={{ display: "grid", gap: 6 }}>
            <div className="helper">
              {postMeta.status || "published"} · /bulletin/{postMeta.slug}
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <button className="btn-red" type="button" onClick={onPublish} disabled={publishBusy}>
                {publishBusy ? "Saving…" : "Edit publish"}
              </button>
              <button className="btn" type="button" onClick={onUnpublish} disabled={publishBusy}>
                Unpublish
              </button>
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            <button className="btn-red" type="button" onClick={onPublish} disabled={publishBusy}>
              {publishBusy ? "Saving…" : "Publish as post"}
            </button>
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        {note?.tags?.length
          ? <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{note.tags.map((tag) => <span key={tag} className="tag">#{tag}</span>)}</div>
      ...
      <div>
        ...
        {backlinks.length
          ? <div style={{ display: "grid", gap: 4 }}>{backlinks.map((item) => <button key={item.id} className="btn" type="button" onClick={() => onOpenNote(item.id)}>{item.title}</button>)}</div>
