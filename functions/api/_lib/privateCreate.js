...
export async function createPrivateOrg({db,request,userId,body}) {
  ...
  const {id,ciphertext,keyCheck,wrappedKey,device_id,recovery,enabled_modules}=body;
  if(Object.keys(body).some(k=>!['private_mode','id','ciphertext','keyCheck','wrappedKey','device_id','recovery','enabled_modules'].includes(k))) return bad(400,'PLAINTEXT_FIELDS_FORBIDDEN');
